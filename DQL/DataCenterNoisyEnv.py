...
class DataCenterNoisyEnv(gym.Env):
    def __init__(self, max_steps_per_episode=100):          # Max steps set to 100 TO EXPERIMENT
        super(DataCenterNoisyEnv, self).__init__()
        ...
        # Probability p, set to 0.1 for now
        self.pt = 0.1

        # Noise level, set to 0.05 for now
        self.noise = 0.05
    ...
    def _get_state(self):
        # Current state without noise
        true_state = np.array([self.memory1, self.cpu1, self.memory2, self.cpu2])

        # Add noise to the state observations
        noisy_state = true_state + np.random.normal(0, self.noise * 100, size=true_state.shape)

        # Ensure the noisy values are within valid bounds
        noisy_state = np.clip(noisy_state, 0, 100)
    ...
    def render(self, mode='human', close=False):
        ...
        print(f"Noisy State: {self.state}")
